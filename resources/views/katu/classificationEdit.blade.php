<x-layouts.app>
    ...
    <div class="ml-77 mt-10 pr-10" x-data="{ note: '' }">
        ...
        <form action="{{ route('classifications.update', $classification->code) }}" method="POST"
            class="bg-[#29627C] rounded-md shadow-xl p-12 w-150 ml-20">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div class="flex flex-col gap-2">
                    ...
                    <select name="parent_code" id="classification_select"
                        class="rounded-md bg-white w-full h-12 px-4 text-lg" required>
                        <option value="" disabled selected>Pilih Kode Surat</option>
                        @foreach ($classifications as $classif)
                            <option value="{{ $classif->code }}"
                                {{ $classif->code == $classification->parent_code ? 'selected' : '' }}>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="text" value="{{ $classification->code }}" id="new_code_input" name="code"
                        class="rounded-md w-full h-12 px-4 text-lg bg-white">
                </div>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="text" value="{{ $classification->name }}" name="name"
                        class="rounded-md bg-white w-full h-12 px-4 text-lg">
                </div>
            </div>
            ...
            <div class="flex justify-center w-full gap-4 mt-10 ">
                ...
                <button type="submit" class="bg-[#28A745] hover:bg-[#218838] text-white text-xl rounded-md px-8 p-2"
                    class="px-8 py-3 text-white text-xl rounded-lg transition font-semibold">
                    Simpan
                </button>
            </div>
